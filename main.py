...
def scan_market(symbol_list):
    ...
    for symbol in symbol_list:
        try:
            ...
            # ✅ 印出趨勢資訊（可搭配 bias 標籤）
            if is_bullish:
                print(f"[資訊] 🟢 偏多 | {symbol} ➜ RSI={rsi:.1f}｜EMA5>{ema5:.2f} > EMA20={ema20:.2f}")
                rrov_conditions = {...}
                mean_revert_conditions = {...}
            elif is_bearish:
                print(f"[資訊] 🔴 偏空 | {symbol} ➜ RSI={rsi:.1f}｜EMA5={ema5:.2f} < EMA20={ema20:.2f}")
                rrov_conditions = {...}
                mean_revert_conditions = {...}
            else:
                print(f"[資訊] ⚪ 中性 | {symbol} ➜ RSI={rsi:.1f}｜EMA5={ema5:.2f} ≈ EMA20={ema20:.2f} ➜ 略過建倉判斷")
                continue
            # ✅ 多單條件（RROV / 均值回歸）
