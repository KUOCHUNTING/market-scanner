...
def enter_position(symbol, price, direction, signal_note, rsi=None, tmo=None, zscore=None):
    global capital_left
    ...
    if isinstance(rsi, pd.Series):
        rsi = rsi.iloc[-1]
    if isinstance(tmo, pd.Series):
        tmo = tmo.iloc[-1]
    if rsi is not None and pd.notna(rsi):
        msg += f"📉 RSI：{rsi:.1f}\n"
    if tmo is not None and pd.notna(tmo):
        msg += f"📉 TMO：{tmo:.2f}\n"
        ...
        try:
            ...
            if signal_type:
                ...
                enter_position(
                    ...
                    signal_note=signal_note,
                    rsi=indicators['rsi'].iloc[-1],
                    tmo=indicators.get('tmo', None),       # 可選
                    zscore=indicators.get('zscore', None)   # 可選
                )
