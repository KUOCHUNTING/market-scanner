...
def get_recent_data(symbol: str, client: SyncStocksClient) -> pd.DataFrame | None:
    """
    先抓當天 15 分 K；拿不到就：
        1. 改抓「分鐘」K (multiplier=1)  → resample 成 15 分
        2. 還是空，就改抓昨天 RTH 的 15 分
    全部都失敗就回 None。
    """
    tz = pytz.timezone("US/Eastern")
    now_et = datetime.now(tz)

    # === step 0: 先決定今天的 RTH 區間 ===
    rth_start = tz.localize(datetime.combine(now_et.date(), time(9, 30)))
    rth_end   = tz.localize(datetime.combine(now_et.date(), time(16, 0)))

    # helper
    def _call(mult, span, start, end):
        return client.get_aggregate_bars(
            symbol, mult, span,
            int(start.timestamp() * 1000),
            int(end.timestamp()   * 1000),
            limit=5000, adjusted=True).results or []

    # === step 1: 直接要 15 分 K ===
    bars = _call(15, "minute", rth_start, rth_end)

    # === step 2: 沒資料 → 用 1 分 K 再重採樣 ===
    if not bars:
        one_min = _call(1, "minute", rth_start, rth_end)
        if one_min:
            df = pd.DataFrame(one_min).set_index("t")
            # Polygon 給的是 ms；轉成秒再 resample
            df.index = pd.to_datetime(df.index, unit="ms", utc=True).tz_convert(tz)
            df_15 = (
                df[["o","h","l","c","v"]]
        ...
        # fall-through 到 step 3

    # === step 3: 今天空 → 改抓昨天 ===
    if not bars:
        y_start = rth_start - timedelta(days=1)
        y_end   = rth_end   - timedelta(days=1)
        bars = _call(15, "minute", y_start, y_end)

    if bars:
        return pd.DataFrame(bars)
    else:
        print(f"[WARNING] {symbol} 兩日皆無 15 分 K")
        return None
