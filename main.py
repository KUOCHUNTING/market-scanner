...
GOOGLE_SHEET_NAME = "交易紀錄"  # 你要寫入的分頁名稱
GOOGLE_CREDENTIAL_FILE = "gsheet_key.json"  # 放在專案根目錄

SCOPES = ["https://www.googleapis.com/auth/spreadsheets"]
credentials = Credentials.from_service_account_file(GOOGLE_CREDENTIAL_FILE, scopes=SCOPES)
gc = gspread.authorize(credentials)
worksheet = gc.open_by_key(GOOGLE_SHEET_ID).worksheet(GOOGLE_SHEET_NAME)

# === API 金鑰與推播設定 ===
POLYGON_API_KEY = os.getenv("POLYGON_API_KEY", "你的預設 Polygon 金鑰")  # ⚠️ 改成你的 KEY 或留空等上層代入
...
def fetch_stock_data(symbol):
    from polygon import RESTClient  # 放在函數內，避免整體卡住
    client = RESTClient(api_key=POLYGON_API_KEY)

    try:
        # ✅ 設定時間區間：延遲15分鐘 → 向前回推2小時
        ...
        print(f"[DEBUG] 抓取 {symbol} 資料時間：{start_time} → {end_time}")

        bars = client.get_aggs(
            symbol=symbol,
            multiplier=5,  # ✅ 5 分K，也可改 30
            timespan="minute",
            from_=from_ts,
            to=to_ts,
            limit=500
...
def fetch_30min_data(symbol):
    ...
    try:
        now_est = datetime.now(EST)
        end_time = now_est - timedelta(minutes=15)
        start_time = end_time - timedelta(days=2)  # 抓2日內的30分K

        from_ts = int(start_time.timestamp())
        to_ts = int(end_time.timestamp())

        print(f"[DEBUG] 抓取共振（30分K）{symbol}：{start_time} → {end_time}")
...
def get_tick_data_from_polygon(api_key, window=20):
    try:
        ...
        end_time = now - timedelta(minutes=15)  # ✅ 延遲15分鐘
        ...
        if hasattr(aggs, "df"):
            df = aggs.df.reset_index()
            tick_series = df['close'].tolist()
            tick_now = tick_series[-1]
            return tick_now, tick_series
        else:
            print("[TICK] ❌ 無法取得 TICK 數據")
            return None, []
...
# === 30 分鐘週期共振判斷 ===
def confirm_30min_resonance(symbol):
    df_30 = fetch_30min_data(symbol)
    if df_30 is None:
        return False  # 無法確認共振

    indicators = apply_indicators(df_30)
    if indicators is None:
        return False
