...
# ✅ 出場判斷邏輯（可放在最前面）
def check_exit_condition(entry_price, current_price, direction):
    TRAIL_TRIGGER = 0.03
    TRAIL_MARGIN = 0.01
    STOP_LOSS = 0.02
    ...
    if pnl <= -STOP_LOSS:
        return True, f"🔻 觸發停損 -2%，自動出場（P&L：{pnl*100:.2f}%）"

    if pnl >= TRAIL_TRIGGER:
        max_gain_price = entry_price * (1 + TRAIL_TRIGGER) if direction == "long" else entry_price * (1 - TRAIL_TRIGGER)
        loss_threshold = max_gain_price * (1 - TRAIL_MARGIN) if direction == "long" else max_gain_price * (1 + TRAIL_MARGIN)

        if (direction == "long" and current_price < loss_threshold) or (direction == "short" and current_price > loss_threshold):
            return True, f"🔔 浮動停利回落超過 1%，自動出場（P&L：{pnl*100:.2f}%）"
...
def run_scanner(tick_series):
    ...
    for symbol in list(open_positions.keys()):
        ...
        should_exit, exit_msg = check_exit_condition(entry_price, current_price, direction)
        if should_exit:
            send_to_discord(f"🚪 自動出場：{symbol}｜{exit_msg}")
    ...
    if not signal_note:
        print(
            f"📌 {symbol}｜收盤：${latest_price:.2f}｜RSI：{latest_rsi:.1f}｜MACD：{latest_macd:.2f}｜"
