...
def get_strategy_match_score(strategy_name, conditions_dict):
    # ✅ 只計算非 None 的條件（有效條件）
    valid_items = {k: v for k, v in conditions_dict.items() if v is not None}
    total = len(valid_items)
    satisfied = sum(1 for cond in valid_items.values() if cond)

    if total == 0:
        print(f"[警告] 策略 {strategy_name} 無條件可供計算 ➜ 預設命中率為 0")
        return 0

    matched = [k for k, v in valid_items.items() if v]
    print(f"[命中率] {strategy_name} ➜ {satisfied}/{total} 命中 → {satisfied/total:.2f} ｜✅條件：{matched}")
