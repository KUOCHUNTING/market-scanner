...
def record_entry_position(symbol, price, direction, shares, strategy_name, confidence_score=None, capital_used=None):
    entry = {
        "symbol": symbol,
        "price": price,
        "direction": direction,
        ...
        "capital_used": capital_used,  # ✅ 新增欄位
...
def push_entry_to_discord(symbol, direction, price, signal_note, zscore=None, rsi=None, roc=None,
    ...
    if strategy == "均值回歸策略":
        ...
        if ema5 is not None and ema20 is not None:
            diff = ema5 - ema20
            content += f"📈 EMA差值：{diff:.2f}（5日 - 20日）\n"
        if rsi is not None:
        ...
        if rsi is not None:
            content += f"📉 RSI：{rsi:.1f}｜"
        if roc is not None:
            content += f"ROC：{roc:.2f}\n"
        if vwap is not None:
            vwap_deviation = abs(price - vwap) / vwap * 100 if vwap > 0 else 0
            content += f"📊 VWAP乖離：{vwap_deviation:.2f}%\n"
        if obv_change is not None:
            content += f"📈 OBV變化：{obv_change:.2f}\n"

    elif strategy == "順勢策略":
        if ema5 is not None and ema20 is not None:
            trend_diff = ema5 - ema20
            content += f"📈 EMA順勢：{trend_diff:.2f}（5日 - 20日）\n"
        if rsi is not None:
            content += f"📉 RSI：{rsi:.1f}\n"
        if obv_change is not None:
            content += f"📈 OBV趨勢變化：{obv_change:.2f}\n"
        if vwap is not None:
...
def enter_position(symbol, price, direction, signal_note, rsi=None, zscore=None, strategy_name="未標記策略",
    ...
    quantity, capital_used = compute_position_size(price)
    
    if quantity <= 0:
        print(f"[跳過] {symbol} 價格過高，無法建倉")
        return

    capital_left -= capital_used  # ✅ 這裡才是正確扣資金的位置
