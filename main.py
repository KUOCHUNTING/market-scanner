...
def get_fundamentals(symbol, polygon_api_key, df=None):  # ✅ 加入 df 參數（optional）
    try:
        url = f"https://api.polygon.io/v3/reference/tickers/{symbol}?apiKey={polygon_api_key}"
        res = requests.get(url)
        data = res.json().get("results", {})

        # 嘗試從 Polygon 拿 avg_volume
        avg_volume_api = float(data.get("average_volume", 0))

        # 若失敗、且 df 有傳入，就自己計算平均成交量
        avg_volume_calculated = 0
        if avg_volume_api == 0 and df is not None and "volume" in df.columns:
            avg_volume_calculated = df["volume"].tail(60).mean()  # 用 60 根（約3天）估算

        avg_volume_final = avg_volume_api if avg_volume_api > 0 else avg_volume_calculated

        fundamentals = {
            "market_cap": float(data.get("market_cap", 0)),
            "avg_volume": avg_volume_final,
