...
def fetch_tick_data(api_key: str, timespan="minute", multiplier=1, limit=150):
    symbol = "TICK"  # 或 "$TICK" / "TICK.NYSE" 依照資料源
    url = f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/{multiplier}/{timespan}/2025-06-14/2025-06-17?apiKey={api_key}&limit={limit}"
    try:
...
def get_tick_dataframe(minutes=30):
    try:
        est = pytz.timezone("US/Eastern")
        now = datetime.now(est)
        start_time = now - timedelta(minutes=minutes)  # 改為參數化
        end_time = now  # ✅ 補上 end_time，避免出錯
        ...
        print(f"[ERROR] get_tick_dataframe 錯誤：{e}")
        return None
...
def check_abnormal_volume(symbol, df):
    ...
    try:
        ...
        tmo = df['tmo'].iloc[-1] if 'tmo' in df.columns else 0
        ...
        if latest_volume > 5 * avg_volume_20:
            ...
            push_to_discord(
                ...
                ema_cross="不明",              # ✅ 可替換為實際 EMA 判斷
                kd_status="不明",              # ✅ 可替換為 KD 金叉/死叉
                candle_type="不明",            # ✅ 可替換為 K 棒型態
                signal_note="⚠️ 異常爆量預警｜" + indicator_text
            )
            ...
            if symbol not in observed_candidates:
                ...
                print(f"[OBSERVE] 🔔 已加入觀察名單：{symbol}")
...
def detect_latent_signal(symbol, df, rsi, tmo, obv, latest_price, latest_vwap):
    def safe_val(val):
        return val.iloc[-1] if isinstance(val, pd.Series) else val

    price    = safe_val(latest_price)
    vwap     = safe_val(latest_vwap)
    rsi_val  = safe_val(rsi)
    tmo_val  = safe_val(tmo)
    obv_val  = safe_val(obv)

    # === 技術指標擷取 ===
    ema5 = df['close'].ewm(span=5, adjust=False).mean().iloc[-1]
    ema20 = df['close'].ewm(span=20, adjust=False).mean().iloc[-1]
    ema_cross = "EMA5 上穿 EMA20" if ema5 > ema20 else "EMA5 下穿 EMA20"

    candle_type = "陽線" if df['close'].iloc[-1] > df['open'].iloc[-1] else "陰線"
    kd_status = "不明"
    obv_direction = "上升" if obv_val > obv.iloc[-3] else "下滑"
    ...
    if price < ema5 and rsi.iloc[-1] > rsi.iloc[-2] and tmo.iloc[-1] > tmo.iloc[-2]:
        add_to_observed_candidates(symbol, price, "RSI 回升 + TMO 黃金交叉")
        signal_note = (
            f"⚠️ **[{symbol}] 潛伏 - 多頭轉折預警**\n"
            f"📉 價格貼近 EMA5，技術面轉強\n"
            f"📊 RSI：{rsi_val:.1f} ↗️｜TMO：{tmo_val:.2f} ↗️｜VWAP：目前位於下方\n"
            f"🕯️ K棒：{candle_type}｜EMA：{ema_cross}\n"
            f"📌 已加入觀察名單，等待正式啟動"
        )
        push_to_discord(
            symbol=symbol,
    ...
    # ✅ 空頭潛伏轉折預警
        ...
        signal_note = (
            f"⚠️ **[{symbol}] 潛伏 - 空頭轉折預警**\n"
            f"📈 價格偏高但動能轉弱，可能轉跌\n"
            f"📊 RSI：{rsi_val:.1f} ↘️｜TMO：{tmo_val:.2f} ↘️｜VWAP：目前位於上方\n"
            f"🕯️ K棒：{candle_type}｜EMA：{ema_cross}\n"
            f"📌 已加入觀察名單，等待正式啟動"
        )
        push_to_discord(
...
def analyze_stock_data(symbol, bars, tick_value, trin_value):
    try:
        ...
        print(f"📈 量能：{volume_ratio:.2f} 倍｜EMA：{ema_cross}｜OBV：{obv_direction}｜KD：{kd_status}｜K棒：{candle_type}")
...
def evaluate_breakout_signal(symbol, df):
    ...
    latest_vwap = (df['close'] * df['volume']).cumsum().iloc[-1] / df['volume'].cumsum().iloc[-1]
    ...
    volume_ratio = volume.iloc[-1] / avg_vol if avg_vol > 0 else 1.0
    ...
    # 🐶 空頭正式進場
        ...
        if not has_confluence_30m:
            ...
            push_to_discord(symbol, signal_note)  # 保留推播
            return  # ✅ 中止正式進場流程
        else:
            signal_note += "\n📈 技術共振：✅ 30M 共振"
...
for symbol in stock_list:
    try:
        ...
        signal_note, auto_entry, latent_direction = detect_latent_signal(
            symbol, df, rsi, tmo, obv, latest_price, latest_vwap
        )
        if signal_note:
            message = (
    ...
    if df is None or df.empty:
        print(f"[跳過] {symbol} 無法取得資料")
        continue

    latest_price = df["close"].iloc[-1]  # ✅ 抓最新收盤價
