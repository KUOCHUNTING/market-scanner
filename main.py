...
def fetch_stock_data(symbol):
    ...
    try:
        end_time = datetime.now(pytz.timezone("US/Eastern")) - timedelta(minutes=5)
...
def fetch_30min_data(symbol):
    ...
    try:
        ...
        print(f"[DEBUG] 抓取時間 from：{start_time.isoformat()} → to：{end_time.isoformat()}")
        ...
        df = pd.DataFrame([{
            ...
            "close": bar.c,
            "volume": bar.v
        } for bar in bars.results if datetime.fromtimestamp(bar.t / 1000) <= end_time])
