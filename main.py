...
# ✅ 使用「symbol + 策略名稱 + 日期」當作 key，避免同一天同策略重複進場
today = datetime.now().strftime("%Y-%m-%d")
entered_key = f"{symbol}_{strategy_name}_{today}"

if entered_key in entered_positions:
    print(f"[跳過] {symbol} ➜ {strategy_name} 已於今日建倉，略過重複進場")
    return
else:
    entered_positions.add(entered_key)
