    ...
    # ✅ 第二次通知（正式建倉 - 空頭）
        ...
        if symbol in observed_candidates:
            ...
            if price_diff <= 0.01 and time_diff <= 20:
                ...
                signal_note = (
                    f"🌪 潛伏空頭（正式建倉）\n"
                    f"📊 RSI：{rsi.iloc[-1]:.1f} ↘️｜TMO：{tmo.iloc[-1]:.2f} ↘️｜OBV：{obv_direction}｜VWAP：跌破｜🕯️ {candle_type}\n"
                    f"📌 確認空頭啟動，建倉時機已到"
                )
...
def analyze_stock_data(symbol, bars):
    try:
        ...
        # 最後印出所有指標，確認有值
        print(f"[DEBUG] {symbol} 指標分析結果：收盤={latest_price:.2f}｜RSI={rsi:.2f}｜TMO={tmo:.2f}｜VWAP={latest_vwap:.2f}｜ADX={adx:.2f  if adxelse -1}")
    ...
    low=df['low'], close=df['close']).plus_di()
    low=df['low'], close=df['close']).minus_di()
