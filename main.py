...
def check_exit_and_notify(symbol, latest_price):
    ...
    # ✅ 再計算損益金額與報酬率百分比
    pnl_raw = (latest_price - entry_price) / entry_price if direction == "多" else (entry_price - latest_price) / entry_price
    pnl_text = f"{pnl_raw * 100:.4f}%"
    pnl_emoji = "🟢" if pnl_raw > 0 else "🔴" if pnl_raw < 0 else "⚪"
    profit_dollar = pnl  # ✅ 這行加上去

    # === 顯示完整資訊 ===
    print(f"✅【持續持有】{symbol}｜{pnl_emoji} 報酬率：{pnl_text}｜進場價={entry_price:.2f}｜目前價={latest_price:.2f}｜策略={strategy}｜已持有 {holding_minutes} 分鐘")
