...
capital_left = round(capital_left, 0)          # 當前剩餘資金
...
def scan_market(symbol_list):
    ...
    for symbol in symbol_list:
        try:
            ...
            print(f"[DEBUG] {symbol} ➜ 策略名稱：{strategy_name}｜emoji：{get_strategy_display(strategy_name)}") 
...
def enter_position(symbol, price, direction, signal_note,
    ...
    positions[symbol] = {
        "direction": direction,
        "entry_price": price,
        "quantity": shares,  # ✅ 修正：用 quantity 統一名稱
        "entry_time": now,
        "capital_used": capital_used,
        "sell_stage": 0,
        "max_gain": 0.0,
        "strategy": strategy_name,
        "strategy_display": strategy_display,  # ✅ 加上 emoji 版策略名稱
        "rsi": rsi,
        "zscore": zscore,
        "ema5": ema5,
        "ema20": ema20,
...
def schedule_exit_check():
    if positions:
        ...
        print("[排程] 無持倉，跳過出場檢查")
    threading.Timer(60, schedule_exit_check).start()  # 60 秒後再次執行自己
