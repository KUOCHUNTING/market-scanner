    ...
    if (
        ...
        if symbol in observed_candidates:
            ...
            if price_diff <= 0.02 and time_diff <= 30:
                ...
                signal_note = (
                    f"🌱 潛伏多頭（正式建倉）\n"
                    f"🔸 RSI：{rsi.iloc[-1]:.1f}｜TMO：{tmo.iloc[-1]:.2f}｜OBV：{obv_direction}\n"
                    f"📊 VWAP：{vwap_status}｜🕯️ {candle_type}\n"
                    f"✅ 確認多頭啟動，建倉時機已到！"
                )
                ...
                if symbol not in entered_positions:
                    ...
                    # ✅ 🔔 在成功建倉後，加入這段推播
                    message = (
                        f"{'🌱' if direction == '多' else '🌪'} 潛伏建倉 - {direction}頭\n"
                        f"🔸 RSI：{rsi:.1f}｜TMO：{tmo:.2f}｜OBV：{obv_direction}\n"
                        f"📊 VWAP：{vwap_status}｜🕯️ {candle_type}\n"
                        f"✅ 確認{direction}頭啟動，建倉時機已到！\n\n"
                        f"💰 投入資金：${capital_used:.0f}｜股數：{shares} 股｜剩餘資金：${capital_left:.0f}\n"
    ...
    if (
        ...
        if symbol in observed_candidates:
            ...
            if price_diff <= 0.02 and time_diff <= 30:
                ...
                signal_note = (
                    f"🌪 潛伏空頭（正式建倉）\n"
                    f"📊 RSI：{rsi.iloc[-1]:.1f} ↘️｜TMO：{tmo.iloc[-1]:.2f} ↘️｜OBV：{obv_direction}｜VWAP：{vwap_status}｜🕯️ {candle_type}\n"
                    f"📌 確認空頭啟動，建倉時機已到"
                )
                ...
                if symbol not in entered_positions:
                    ...
                    message = (
                        f"{emoji} 潛伏建倉 - 空頭\n"
                        f"🔸 RSI：{rsi:.1f} {arrow_rsi}｜TMO：{tmo:.2f} {arrow_tmo}｜OBV：{obv_status}\n"
                        f"📊 VWAP：{vwap_status_text}｜🕯️ {candle_type}\n"
                        f"✅ 確認空頭啟動，建倉時機已到！\n\n"
                        f"💰 投入資金：${capital_used:.0f}｜股數：{shares} 股｜剩餘資金：${capital_left:.0f}\n"
                        f"🕑 時間：{now.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
...
def check_30min_confluence(df_30m, direction="long"):
    try:
        ...
        if direction == "long":
            if rsi < 40: signal_strength += 1
            if tmo_diff > 0: signal_strength += 1
            if df_30m['close'].iloc[-1] > latest_vwap: signal_strength += 1
            if obv_direction == "上升": signal_strength += 1
        else:  # 空頭
            if rsi > 60: signal_strength += 1
            if tmo_diff < 0: signal_strength += 1
            if df_30m['close'].iloc[-1] < latest_vwap: signal_strength += 1
            if obv_direction == "下降": signal_strength += 1
