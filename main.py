...
def scan_market(symbol_list):
    ...
    for symbol in symbol_list:
        try:
            ...
            # ✅ 印出整合資訊
            print(f"[資訊] {bias}|{symbol} ➜ 價格=${latest_price:.2f}|RSI={rsi:.1f}|ROC={roc:.2f}|OBV={int(obv):,}|VWAP={vwap:.2f}|Z-score={zscore:.2f}|EMA5={ema5:.2f}|EMA20={ema20:.2f}|{ema_summary}")
            ...
            # === 技術傾向判斷（僅供參考）===
            bias = "⚪ 中性"
            if rsi > 50 or roc > 0.5 or ema5 > ema20 or obv_diff > 0:
                bias = "🟢 技術偏多（僅供參考）"
            elif rsi < 40 or roc < -0.5 or ema5 < ema20 or obv_diff < 0:
                bias = "🔴 技術偏空（僅供參考）"

            print(f"[技術傾向] {bias}｜{symbol} ➜ 價格=${latest_price:.2f}｜RSI={rsi:.1f}｜ROC={roc:.2f}｜OBV={int(obv):,}｜VWAP={vwap:.2f}｜Z-score={zscore:.2f}｜EMA5={ema5:.2f}｜EMA20={ema20:.2f}")

            # === 策略方向判斷 ===
            is_bullish = rsi < 55 and ema5 > ema20
            is_bearish = rsi > 45 and ema5 < ema20

            if is_bullish:
                print(f"[策略方向] 🟢 偏多（進場考慮）| {symbol} ➜ RSI={rsi:.1f}｜EMA5={ema5:.2f} > EMA20={ema20:.2f}")
            elif is_bearish:
                print(f"[策略方向] 🔴 偏空（進場考慮）| {symbol} ➜ RSI={rsi:.1f}｜EMA5={ema5:.2f} < EMA20={ema20:.2f}")
            else:
                print(f"[策略方向] ⚪ 中性（跳過建倉）| {symbol} ➜ RSI={rsi:.1f}｜EMA5={ema5:.2f} ≈ EMA20={ema20:.2f}")
                continue
            # ✅ 多單條件（RROV / 均值回歸）
