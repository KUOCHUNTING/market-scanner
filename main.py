...
WEBHOOK_URL = os.getenv("DISCORD_WEBHOOK_URL", "https://discord.com/api/webhooks/1385222120321187850/_qzr0Jq0JP7WtXRFHQcs-l0-kzYg0k6GjrT4J2V8mf9zWqaMFw9SZMbtJsIt7LGOptI6")
...
def push_to_discord(
    ...
    vwap_deviation=None,
    bb_deviation=None,
    content=None  # ✅ 加入純文字內容支援
):
    import requests

    webhook_url = "https://discord.com/api/webhooks/1385222120321187850/_qzr0Jq0JP7WtXRFHQcs-l0-kzYg0k6GjrT4J2V8mf9zWqaMFw9SZMbtJsIt7LGOptI6"  # 🔁 換成你實際的 URL
    ...
    try:
        ...
        bb_dev_text = f"{bb_deviation:.2f}%" if bb_deviation is not None else "N/A"
        vwap_dev_text = f"{vwap_deviation:.2f}%" if vwap_deviation is not None else "N/A"
        ...
        content = (
            ...
            f"📊 {signal_note}\n"
            f"💵 價格：${price:.2f}｜方向：{direction}｜策略：{strategy_name}\n"
            f"📈 RSI={rsi_text}｜ROC={roc_text}｜Z-score={zscore_text}\n"
            f"📉 VWAP={vwap_text}｜VWAP乖離={vwap_dev_text}｜布林乖離={bb_dev_text}\n"
            f"📦 OBV={obv_text}（Δ={obv_change:.1f}）｜成交量：{volume_text}\n"
            f"🔬 EMA交叉：{ema_cross}｜K棒型態：{candle_type}｜KD：{kd_status}\n"
            f"🤖 信心分數：{confidence_text}"
        )

        # ✅ 發送
        requests.post(webhook_url, json={"content": content})
        print(f"[✅推播] {symbol} ➜ {signal_type} 推播已發送")
...
def detect_trading_signal(symbol, df, indicators, debug=False, force_test=False):
    ...
    curr_volume = indicators['volume'].iloc[-1]
