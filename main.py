...
def scan_market(symbol_list):
    ...
    for symbol in symbol_list:
        try:
            ...
            if len(df_raw) < 27:
                print(f"[跳過] {symbol} ➜ 資料不足（少於 27 根）")
                continue
        ...
        if (rsi > 60 or roc > 0.5 or ema5 > ema20 or obv_diff > 0):
            ...
            # === ⛔ 沒進場，但有診斷理由，就推播診斷訊息
                ...
                content = (
                    ...
                    f"📉 價格=${latest_price:.2f}｜RSI={indicators['rsi'].iloc[-1]:.1f}｜"
                    f"Z-score={indicators['zscore'].iloc[-1]:.2f}｜EMA5={indicators['ema_5'].iloc[-1]:.2f}｜EMA20={indicators['ema_20'].iloc[-1]:.2f}"
                )
