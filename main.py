...
def fetch_stock_data(symbol):
    ...
    try:
        ...
        est = pytz.timezone("US/Eastern")
        start_time = est.localize(datetime(2024, 6, 3, 10, 0))  # 測試資料有開盤的時間
        end_time = est.localize(datetime(2024, 6, 3, 12, 0))
        ...
        if aggs is None or not hasattr(aggs, "df") or aggs.df.empty:
...
def fetch_30min_data(symbol):
    ...
    try:
        ...
        df = aggs.df if hasattr(aggs, "df") else None

        if df is None or df.empty:
            print(f"[⚠️警告] 共振資料無效：{symbol}")
            return None
