...
def fetch_15min_stock_data(symbol, include_yesterday=True):
    ...
    client = RESTClient(api_key="3Oa52hFieaUvTyToZudJanq39Rw9zApi")  # ⬅️ 這就是你圖上那組 API 金鑰
    ...
    try:
        ...
        print(f"[錯誤] 抓取 15def fetch_stock_data(symbol, api_key="你的_API_KEY"):
    try:
        # 建立 Polygon client
        ...
        # ✅ 設定抓取當天盤中 9:30 ~ 16:00
        start_dt = est.localize(datetime.combine(today, time(9, 30)))
        end_dt = est.localize(datetime.combine(today, time(16, 0)))

        # ✅ 時間格式轉換為 Polygon API 接受的 ISO 格式
        from_str = start_dt.strftime("%Y-%m-%dT%H:%M:%S%z")
        to_str = end_dt.strftime("%Y-%m-%dT%H:%M:%S%z")

        print(f"[DEBUG] 抓取 {symbol} 15 分K：{from_str} → {to_str}")
