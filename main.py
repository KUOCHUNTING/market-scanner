...
def fetch_stock_data(symbol, api_key):
    ...
    try:
        ...
        # 🟡 加入這段：如果資料不足，就往前補抓最多 3 個交易日
        retry_days = 0
        while len(df) < 30 and retry_days < 3:
            retry_days += 1
            print(f"[補抓] {symbol} 資料不足（{len(df)} 根），嘗試往前第 {retry_days} 天")

            prev_day = start_time.date() - timedelta(days=1)
            while retry_days < 3 and len(df) < 30:
                retry_days += 1
            ...
            print(f"[補抓] 抓取 {symbol} 第 {retry_days} 天 ➜ {start_time} → {end_time}")
            ...
            if bars:
                ...
                print(f"[補抓] 第 {retry_days} 天仍無資料，繼續下一天")

        print(f"[DEBUG] {symbol} 補抓後 df.shape：{df.shape}")
        
        if df.empty:
            print(f"[⚠️ 警告] {symbol} bars 轉換後是空的 DataFrame")
            return None
...
def detect_trading_signal(symbol, df, indicators):  # ✅ 有 symbol 參數
    ...
    if signal_type is None:
        print(f"[調試] {symbol} 未觸發建倉條件 ➜ RSI={rsi:.1f}, ROC={roc:.2f}, OBV變化={obv - indicators['obv'].iloc[-2]:.2f}, VWAP乖離={(abs(latest_price - vwap)/vwap)*100:.2f}%")    
...
def scan_market(symbol_list):
    for symbol in symbol_list:
        try:
            ...
            # === ✅ 策略一：均值回歸策略（自動建倉與推播）
            signal_note1 = detect_mean_reversion_signals(df, symbol)

            # === ✅ 策略二：多空共振策略（手動處理建倉）
            signal_type, signal_note2 = detect_trading_signal(symbol, df, indicators)

            if signal_type:
                direction = "多" if signal_type == "BUY" else "空"
                rsi = indicators.get("rsi", pd.Series()).iloc[-1] if "rsi" in indicators else None
                zscore = indicators.get("zscore", pd.Series()).iloc[-1] if "zscore" in indicators else None

                confidence_score = compute_confidence_score(rsi=rsi, zscore=zscore)
                ...
                # ✅ 資金條件檢查
                if not can_enter_new_position(symbol, capital_required):
                    continue
