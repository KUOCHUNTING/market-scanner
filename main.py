...
def fetch_stock_data(symbol):
    try:
        ...
        if bars_response is None or not hasattr(bars_response, "df"):
            print(f"[ERROR] {symbol} 回傳資料異常（None 或無 df）")
            return None

        bars = bars_response.df
        if bars.empty or 'close' not in bars.columns or len(bars) < 15:
...
print(f"📊 掃描開始，共 {len(stock_list)} 檔")
...
for symbol in stock_list:
    ...
    try:
        ...
        print(f"[INFO] {symbol} K線筆數：{len(df)}，開始技術分析")
        ...
        tmo_cross = prev_tmo < 0 and latest_tmo > 0
        mfi = MFIIndicator(high=df['high'], low=df['low'], close=df['close'], volume=df['volume']).money_flow_index()
        latest_mfi = mfi.iloc[-1]
        prev_mfi = mfi.iloc[-2]
        mfi_down = prev_mfi > latest_mfi and prev_mfi > 60

        # ✅ 小資過濾
        market_cap = get_market_cap(symbol)
        avg_volume_10d = df['volume'].tail(10).mean()
        atr_3d = AverageTrueRange(high=df['high'], low=df['low'], close=df['close'], window=3).average_true_range().iloc[-1]
