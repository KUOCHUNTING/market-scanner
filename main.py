...
def check_abnormal_volume(symbol, df):
    ...
    try:
        ...
        if latest_volume > 5 * avg_volume_20:
            ...
            message = (
                f"⚠️ **[💣💣異常爆量警告💣💣]** {symbol}\n"
                f"📈 價格：${df['close'].iloc[-1]:.2f}｜成交量：{latest_volume:,} 股\n"
                f"🧪 過去20根平均量：{avg_volume_20:,.0f} 股｜倍數：{latest_volume / avg_volume_20:.1f}x\n"
    ...
    if (
        ...
        if symbol in observed_candidates:
            ...
            if price_diff <= 0.02 and time_diff <= 30:
                ...
                signal_note = (
                    f"🌱 潛伏多頭（正式建倉）\n"
                    f"📊 RSI：{rsi:.1f} {rsi_trend}｜TMO：{tmo:.2f} {tmo_trend}｜OBV：{obv_direction}｜VWAP偏離：{vwap_deviation:.2%}｜🕯️ {candle_type}\n"
                    f"✅ 確認多頭啟動，建倉時機已到！"
                )
...
def fetch_stock_data(symbol, limit=100):
    try:
        ...
        bars = api.get_bars(symbol, TimeFrame.Minute, start=start, end=end).df
...
def run_scanner(tick_series):
    ...
    if "爆量" in info['reason'] or info['reason'] == "異常爆量觀察":
        try:
            df = fetch_stock_data(symbol)  # 你原本的抓資料函數
            ...
            if (
                latest_rsi > 65 and rsi.iloc[-2] > rsi.iloc[-1] and        # RSI 高檔轉弱
                tmo.iloc[-2] > 0 and tmo.iloc[-1] < 0 and                  # TMO 由正轉負（動能翻空）
                mfi.iloc[-2] > mfi.iloc[-1] and mfi.iloc[-2] > 60 and      # MFI 從高檔轉弱（但未必破50）
                obv.iloc[-1] < obv.iloc[-3] and                            # OBV 顯示資金流出
                candle_type in ['shooting_star', 'bearish_engulfing']     # 空方反轉K棒
            ):
                message = (
                    f"⚠️ **[多轉空 - 爆量反轉預警]** ⚠️ {symbol}\n"
                    f"📉 價格：${df['close'].iloc[-1]:.2f}｜異常爆量見頂疑慮\n"
                    f"📊 RSI：{latest_rsi:.1f} ↘️｜TMO：{tmo.iloc[-1]:.2f}（正轉負）｜MFI：{mfi.iloc[-1]:.1f} ↘️\n"
                    f"📉 OBV：資金流出｜K棒：{candle_type}\n"
                    f"📦 成交量：{df['volume'].iloc[-1] / df['volume'].iloc[-21:-1].mean():.1f}x｜20根均量：{avg_volume_20:,.0f} 股\n"
                    f"🕒 時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                push_to_discord(symbol, message)
                del observed_candidates[symbol]
            ...
            print(f"[ERROR] {symbol} 檢查爆量轉空時出錯：{e}")
...
for symbol in stock_list:
    try:
        ...
        atr_3d = ta.volatility.AverageTrueRange(
            high=df['high'], low=df['low'], close=df['close'], window=3
        ).average_true_range().iloc[-1]

        if not filter_stock_conditions(symbol, latest_price, market_cap, avg_volume_10d, atr_3d):
            print(f"[SKIP] {symbol} 不符合小資條件，跳過")
            continue
...
def fetch_30min_data(symbol):
    try:
        ...
        df = api.get_bars(symbol, TimeFrame.Hour, start=start_time.isoformat(), end=end_time.isoformat()).df
        df = df[df['exchange'] == 'Q']
        df = df.reset_index()
