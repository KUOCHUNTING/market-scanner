...
def generate_daily_summary():
    try:
        # ✅ 使用 .env 的 base64 認證金鑰
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        encoded_key = os.getenv("GOOGLE_SERVICE_ACCOUNT_BASE64")
        creds_dict = eval(base64.b64decode(encoded_key).decode("utf-8"))
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        client = gspread.authorize(creds)

        # ✅ 開啟你的 Google Sheets
        sheet = client.open_by_key(os.getenv("GOOGLE_SPREADSHEET_ID")).worksheet("交易紀錄")
...
def fetch_stock_data(symbol):
    ...
    try:
        # ✅ 設定時間區間：抓延遲 15 分鐘的 5 分K，回推 2 小時
        est = timezone("US/Eastern")
        now_est = datetime.now(est)
        ...
        print(f"[DEBUG] 抓取時間 from：{start_time} → to：{end_time}")
...
def fetch_30min_data(symbol):
    ...
    try:
        ...
        print(f"[DEBUG] 嘗試抓取 30分K：{symbol}")
        print(f"[DEBUG] 抓取時間 from：{from_ts} → to：{to_ts}")
