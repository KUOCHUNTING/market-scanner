...
now_est = datetime.now(est)  # ✅ 補上這行

# === 🌐 Discord Webhook 設定 ===
DISCORD_WEBHOOK_URL = os.getenv("DISCORD_WEBHOOK_URL", "你的預設值")
...
def fetch_stock_data(symbol, api_key):
    ...
    # ✅ 預設抓近 250 分鐘（50 根 5分K）
    end_time = min(now_est, market_close)
    ...
    try:
        ...
        bars = client.get_aggs(
            ...
            multiplier=5,
            timespan="minute",
            from_ts = int(start_time.timestamp() * 1000)  # ✅ Polygon 要求是毫秒
        ...
        if df.empty:
            print(f"[⚠️ 警告] {symbol} 抓到的 bars 是空的 DataFrame")
            return None
