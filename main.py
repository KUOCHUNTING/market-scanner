...
def write_exit_to_sheet(
    ...
    try:
        ...
        row = [
            ...
            f"{holding_minutes} 分鐘" if holding_minutes is not None else "N/A",
...
def enter_position(symbol, price, direction, signal_note,
    ...
    # ✅ 防呆判斷：價格 / 股數 / 資金不能為 0
    if price <= 0 or shares <= 0 or capital_used <= 0:
...
def check_exit_and_notify(symbol, latest_price):
    ...
    repair_position(symbol)  # ✅ 防止 KeyError
...
def exit_position(symbol, current_price, position_data):
    ...
    write_exit_to_sheet(
        ...
        strategy_name=position_data.get("strategy_display", "未知策略")  # ✅ 補這個欄位名稱
    )
