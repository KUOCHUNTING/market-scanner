...
def calculate_exit_metrics(entry_price, exit_price, shares, entry_time, exit_time):
    """
    自動計算出場三大指標：
    1. 報酬率 (%)
    2. 損益金額 (USD)
...
entered_positions = set()  # ✅ 用來記錄哪些股票已建倉，避免重複
...
def record_entry_position(symbol, price, direction, shares, strategy_name,
    ...
    """
    建倉紀錄函數，用於儲存已進場部位資訊。
    若該 symbol 已存在，會顯示警告但仍可覆蓋。
    """
    from datetime import datetime

    # 🧠 可選：檢查是否重複建倉
    if symbol in positions:
        print(f"[⚠️警告] {symbol} 已存在持倉紀錄，將覆蓋舊資料")
    ...
    positions[symbol] = entry  # 寫入全域持倉 dict

    # ✅ 處理顯示格式
    confidence_text = f"{confidence_score:.2f}" if confidence_score is not None else "N/A"
    capital_text = f"${capital_used:,.0f}" if capital_used is not None else "N/A"

    # ✅ 完整日誌
    print(
        f"📝 [建倉紀錄] {symbol} | 價格=${price:.2f}｜股數={shares}｜方向={direction}｜策略={strategy_name}"
        f"｜信心={confidence_text}｜投入={capital_text}"
    )
...
def get_strategy_match_score(strategy_name, conditions_dict):
    # 印出每個策略的條件狀態（方便除錯）
    print(f"[DEBUG] {strategy_name} 條件：{conditions_dict}")

    # 移除值為 None 的條件（如技術指標還沒出來）
    valid_conditions = {k: v for k, v in conditions_dict.items() if v is not None}

    # 避免除以 0，若無任何可用條件則回傳 0
    if not valid_conditions:
        print(f"[警告] 策略 {strategy_name} 無條件可供計算 ➜ 預設命中率為 0")
        return 0

    # 計算命中率（True 的數量 / 條件總數）
    score = sum(valid_conditions.values()) / len(valid_conditions)
    print(f"[評估] {strategy_name} 命中率：{score:.2f}")
    return score
...
def load_stock_list(filepath="filtered_us_stocks_common_only.csv"):
    try:
        df = pd.read_csv(filepath, header=None)  # ❗ 沒有標題列
...
def scan_market(symbol_list):
    ...
    for symbol in symbol_list:
        ...
        try:
            ...
            # === 2. 技術指標分析
            indicators = calculate_indicators(df)  # ✅ 改用正確的變數名稱
            ...
            if signal_type:
                ...
                # ✅ 防重複建倉（若今天已建倉同策略，略過）
                if already_entered_today(symbol, strategy_name):
                    print(f"[跳過] {symbol} ➜ 已建倉 {strategy_name}，略過重複進場")
                    continue
                ...
                strategy_name = "均值回歸"  # ❗對應到 map 的 key
                strategy_display = strategy_label_map.get(strategy_name, "未知策略")
                ...
                # 判斷純多或空
                pure_direction = "多" if "多" in direction else "空"
                # 發送建倉推播
                push_entry_to_discord(
                    symbol=symbol,
                    direction=pure_direction,
                    price=latest_price,
                    signal_note=signal_note,
                    zscore=zscore,
                    rsi=rsi,
                    ...
                    df=df  # ✅ 必須傳入 df 才能跑 analyze_ema_trend()
                )
        ...
        if (rsi > 60 or roc > 0.5 or ema5 > ema20 or obv_diff > 0):
            ...
            # ✅ 多單條件(RROV策略)
            cond_rsi_long = rsi is not None and pd.notna(rsi) and indicators['rsi'].iloc[-2] is not None and rsi < 35 and rsi > indicators['rsi'].iloc[-2]
            cond_roc_long = roc is not None and indicators['roc'].iloc[-2] is not None and roc < 0 and roc > indicators['roc'].iloc[-2]
            cond_obv_long = obv is not None and indicators['obv'].iloc[-2] is not None and obv > indicators['obv'].iloc[-2]
            cond_vwap_near = vwap is not None and abs(latest_price - vwap) / vwap < 0.05
            # ✅ 多單條件(均值回歸策略)
            cond_price_low = 'lower_band' in indicators and indicators['lower_band'].iloc[-1] is not None and latest_price < indicators['lower_band'].iloc[-1]
            cond_rsi_rebound = rsi is not None and indicators['rsi'].iloc[-2] is not None and rsi > indicators['rsi'].iloc[-2] and rsi < 35
            cond_zscore_low = zscore is not None and zscore < -2
            cond_ema_cross = ema5 is not None and ema20 is not None and ema5 > ema20

            # ✅ 空單條件(RROV策略)
            cond_rsi_short = rsi is not None and indicators['rsi'].iloc[-2] is not None and rsi > 65 and rsi < indicators['rsi'].iloc[-2]
            cond_roc_short = roc is not None and indicators['roc'].iloc[-2] is not None and roc > 0 and roc < indicators['roc'].iloc[-2]
            cond_obv_short = obv is not None and indicators['obv'].iloc[-2] is not None and obv < indicators['obv'].iloc[-2]
            # ✅ 空單條件(均值回歸策略)
            cond_price_high = 'upper_band' in indicators and indicators['upper_band'].iloc[-1] is not None and latest_price > indicators['upper_band'].iloc[-1]
            cond_rsi_drop = rsi is not None and indicators['rsi'].iloc[-2] is not None and rsi < indicators['rsi'].iloc[-2] and rsi > 65
            cond_zscore_high = zscore is not None and zscore > 2
            cond_ema_death = ema5 is not None and ema20 is not None and ema5 < ema20
            ...
            if is_bullish:
                ...
                trend_follow_conditions = {
                    "EMA多頭排列": ema5 > ema20 if ema5 and ema20 else None,
                    "RSI強勢": rsi > 55 if rsi else None,
                    "OBV上升": obv_diff > 0 if obv_diff else None,
                    "價格在VWAP上方": latest_price > vwap if vwap else None,
                }
                ...
                mean_revert_conditions = {
                    ...
                    "RSI下滑": cond_rsi_drop,
                    "Z-score超漲": cond_zscore_high,
                    "EMA死叉": cond_ema_death,
                }

                trend_follow_conditions = {
                    "EMA空頭排列": ema5 < ema20 if ema5 and ema20 else None,
                    "RSI弱勢": rsi < 45 if rsi else None,
                    "OBV下降": obv_diff < 0 if obv_diff else None,
                    "價格在VWAP下方": latest_price < vwap if vwap else None,
                }

            else:
                # 📌 中性整理區 ➜ 僅觀察均值回歸策略（不建倉也能評分與推播）
                print(f"[中性偏盤整] {symbol} ➜ 僅執行均值回歸策略比對")
            ...
            # ✅ 放這裡！印出當前命中率診斷
            print(f"[診斷] {symbol} ➜ RROV={rrov_score:.2f}｜均值回歸={mean_score:.2f}｜順勢策略={trend_score:.2f}")

            # ✅ 加這段判斷：沒有任何策略命中，就略過推播
            if rrov_score == 0 and mean_score == 0 and trend_score == 0:
                print(f"[略過] {symbol} ➜ 無任何策略命中，不推播")
                return  # ← 提前跳出，不推播也不建倉
            ...
            elif rrov_score > mean_score and rrov_score > trend_score:
                strategy_name = "RROV"
                print(f"[策略選擇] {symbol} ➜ 使用 RROV（命中 {rrov_score*100:.0f}%）")
            elif mean_score > rrov_score and mean_score > trend_score:
                strategy_name = "均值回歸"
            ...
            if strategy_name == "順勢策略":
                direction = "多" if ema5 > ema20 else "空"
            elif strategy_name == "RROV":
                direction = "多" if cond_rsi_long else "空"
            elif strategy_name == "均值回歸":
                direction = "多" if cond_zscore_low else "空"
            else:
                direction = "無"
            ...
            strategy_display = strategy_label_map.get(strategy_name, strategy_name)

            # === ⚠️ 若完全沒命中策略 ➜ 發送診斷推播
            if strategy_name == "策略未命中":
            ...
            if signal_type1 in ["ALERT_BUY", "ALERT_SELL"]:
                ...
                if latest_price > indicators['upper_band'].iloc[-1]:
                    bb_dev = (latest_price - indicators['upper_band'].iloc[-1]) / indicators['upper_band'].iloc[-1] * 100
                elif latest_price < indicators['lower_band'].iloc[-1]:
                    bb_dev = (latest_price - indicators['lower_band'].iloc[-1]) / indicators['lower_band'].iloc[-1] * 100
            ...
            if signal_type1 in ["BUY", "SELL"]:
                ...
                # === ✅ 建倉資金與股數限制（只在訊號成立時才會扣）
                capital_required = min(
                    TOTAL_CAPITAL * POSITION_RATIO,
...
def push_entry_to_discord(symbol, direction, price, signal_note, zscore=None, rsi=None, roc=None,
    ...
    # === 🛡️ 防止重複推播 ===
    global entered_positions
    if symbol in entered_positions:
        print(f"[跳過推播] {symbol} 已推播，略過重複通知")
        return
...
def enter_position(symbol, price, direction, signal_note, rsi=None, zscore=None, strategy_name="未標記策略",
    ...
    global capital_left, entered_positions  # ✅ 加上 entered_positions！
    ...
    # ✅ 使用對照表轉換為美化版策略名稱
    strategy_type = strategy_label_map.get(strategy_name, strategy_name)
...
def exit_position(symbol, current_price, position_data):
    ...
    write_exit_to_sheet(
        ...
        strategy_name=position_data.get("strategy_name", "未知策略")
    )
...
def main_loop():
    ...
    while True:
        ...
        print(f"\n🔄 [第 {round_count} 輪掃描] {now_str}")

        symbol_list = load_stock_list()

        # 防呆：若清單為空就跳出
        if not symbol_list:
            print("⚠️ [警告] 股票清單為空，停止掃描")
            break

        print(f"[DEBUG] 讀到股票清單 {len(symbol_list)} 檔，前 10 檔：{symbol_list[:10]}")

        scan_market(symbol_list)

        print(f"✅ [第 {round_count} 輪完成] 等待 60 秒...\n")
        time.sleep(60)
