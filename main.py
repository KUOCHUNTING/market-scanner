...
def scan_market(symbol_list):
    ...
    print(f\"[DEBUG] ✅ 建倉成功 ➜ {symbol} → {positions[symbol]}\")
...
def check_exit_and_notify(symbol, latest_price):
    ...
    print(f"[持倉報酬] {symbol}｜策略：{strategy}｜方向：{direction}｜報酬率：{pnl_emoji} {pnl_text}")
...
def check_all_positions():
    print(f\"[DEBUG] 現有持倉列表 ➜ {list(positions.keys())}\")
    if not positions:
...
def schedule_exit_check():
    ...
    for symbol, pos in positions.items():
        ...
        if elapsed_seconds < 30:
            print(f\"[⏳ 冷卻中] {symbol} ➜ 持有 {elapsed_seconds:.1f} 秒，略過出場\")
            return
            elapsed_seconds = (datetime.now() - entry_time).total_seconds()
            if elapsed_seconds < 30:
