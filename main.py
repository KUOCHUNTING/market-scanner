...
def check_exit_and_notify_dynamic(symbol, latest_price, now):
    ...
    if should_exit:
        ...
        capital_left += entry_data['capital_used'] * exit_ratio
        entry_data['holding_ratio'] -= exit_ratio

        print(f"[出場] {symbol} @ {exit_price:.2f}｜比例：{exit_ratio*100:.0f}%｜{reason}")
...
def run_scanner(tick_series):
    # ✅ 每次掃描前，先檢查持倉是否該出場
    for symbol in list(positions.keys()):  # ✅ 改用新版持倉 dict
