...
def filter_fundamentals(symbol, fundamentals):
    ...
    is_low_price = fundamentals.get("price", 5) < 1
    is_delisted = fundamentals.get("is_delisted", False)
    is_recent_earning = fundamentals.get("is_recent_earning", False)

    if market_cap < 300_000_000:
        return False, "❌ 市值過小，易受主力控盤"
    if avg_volume < 5_000_000:
        return False, "❌ 流動性不足（平均成交量 < 500萬股）"
    if is_otc or is_low_price:
        return False, "❌ OTC 或低價股，風險高"
    if is_delisted:
        return False, "❌ 公司已下市或停牌"
    if is_recent_earning:
        return False, "⚠️ 財報發布期，波動較大"
