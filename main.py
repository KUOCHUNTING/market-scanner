...
def fetch_stock_data(symbol):
    ...
    try:
        ...
        if end_time <= market_open or start_time >= market_close:
            print(f"[SKIP] {symbol} 不在盤中（{start_time.strftime('%H:%M')} → {end_time.strftime('%H:%M')}），跳過")
            return None
...
def fetch_30min_data(symbol):
    ...
    try:
        ...
        print(f"[DEBUG] 抓取共振 30 分K {symbol}：{start_time} → {end_time}")
        ...
        print(f"[錯誤] 抓取共振 30 分K {symbol} 發生問題：{e}")
        return None
...
def get_tick_data_from_polygon(api_key, window=20):
    ...
    try:
        ...
        if now <= market_open or now >= market_close:
            print(f"[SKIP] TICK 現在是 {now.strftime('%H:%M')}，不在盤中，跳過")
            return None, []
        ...
        if hasattr(aggs, "df") and not aggs.df.empty:
            df = aggs.df.reset_index()
