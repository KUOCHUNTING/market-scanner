...
def init_sheets():
    ...
    try:
        ...
        for name, headers in pages.items():
            if name in existing_sheets:
                print(f"[INFO] 工作表已存在：{name}")
            else:
                print(f"[CREATE] 建立工作表：{name}")
                new_sheet = spreadsheet.add_worksheet(title=name, rows="1000", cols=str(len(headers)))
                new_sheet.append_row(headers)
                print(f"[INIT] 寫入表頭：{name} ✅")
...
def check_abnormal_reversal_entry(symbol, df):
    try:
        ...
        if all([is_rsi_weakening, is_tmo_rolling_over, is_vwap_near, is_obv_falling, is_red_candle]):
            ...
            entry_amount = 5000
            entry_shares = int(entry_amount / latest_price)
            ...
            msg = (
                ...
                f"📦 進場張數：{entry_shares} 股｜資金投入：約 ${entry_amount:,.2f}\n"
                f"🕯️ 紅K｜時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            send_to_discord(msg)
            ...
            msg = (
                f"🚨 **[進場通知 - 空單]** {symbol}\n"
                f"📉 進場價格：${entry_price:.2f}｜股數：{entry_shares}｜資金：${entry_amount:,.2f}\n"
                f"📌 訊號：爆量觸頂轉折｜時間：{entry_time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
            send_to_discord(msg)
...
def get_tick_series(minutes=30):
    try:
        ...
        # ✅ 砍掉秒與微秒，保持格式為：2025-06-09T10:20:00-04:00
        ...
        print(f"[DEBUG] TICK from: {from_ts} to: {to_ts}")

        # ✅ 自動重試最多 3 次，避免暫時性 API 回空或封鎖
        for attempt in range(3):
            try:
                ...
                print(f"[RETRY-{attempt+1}] {symbol} API 失敗，1秒後重試：{api_error}")
                time.sleep(1)
        else:
            # 🔴 三次都失敗就結束
            print(f"[ERROR] {symbol} 連續三次 API 失敗，放棄")
            return None

        # ✅ 使用 .df（新版本支援）
        if hasattr(aggs, "df") and aggs.df is not None and not aggs.df.empty:
        ...
        # ✅ 備援解析 aggs.results
        bars = getattr(aggs, "results", None)
        if not bars:
            print("[ERROR] TICK 無 bars 結構或為空")
            return None
...
def detect_latent_signal(df, rsi, tmo, obv, latest_price, latest_vwap):
    ...
    # ✅ 正式建倉（多頭）
    if (
        df['close'].iloc[-1] < df['close'].iloc[-3] and  # 價格回跌
        rsi.iloc[-2] < 30 and rsi.iloc[-1] > rsi.iloc[-2] and  # RSI 回升
        tmo.iloc[-2] < 0 and tmo.iloc[-1] > tmo.iloc[-2] and tmo.iloc[-1] < 0 and  # TMO 翻揚未突破0
        obv.iloc[-1] > obv.iloc[-2] > obv.iloc[-3] and obv.iloc[-1] < 0 and  # OBV 三連升但仍為負
        df['close'].iloc[-1] > df['close'].iloc[-2] and  # 近兩根價格上漲
        abs(price - latest_vwap) / latest_vwap < 0.02 and  # VWAP 乖離 < 2%
        ...
        if symbol in observed_candidates:
            ...
            if price_diff <= 0.02 and time_diff <= 30:
                ...
                # ✅ 資金計算區
                capital_used = 10000
                shares = int(capital_used / price)
                remaining_capital = total_capital - sum([pos['capital_used'] for pos in positions.values()])

                # ✅ 建倉推播訊息
                signal_note = (
                    f"🐮 **潛伏多頭（正式建倉）** 🐮 {symbol}\n"
                    f"📈 價格：${price:.2f}｜K棒：陽線｜動能轉強\n"
                    f"📊 RSI：{rsi.iloc[-1]:.1f} ⬆️｜TMO：{tmo.iloc[-1]:.2f} ⬆️（尚未突破0）｜OBV：三連升\n"
                    f"📏 VWAP 偏離：{abs(price - latest_vwap) / latest_vwap:.2%}（貼近主力成本）\n"
                    f"💰 建倉金額：${capital_used:,.0f}｜張數：{shares} 股｜剩餘資金：${remaining_capital:,.0f}\n"
                    f"📌 技術面轉強，底部翻揚，建倉時機成立\n"
    ...
    # ✅ 正式建倉（空頭）
    if (
        df['close'].iloc[-1] > df['close'].iloc[-3] and  # 價格回漲
        rsi.iloc[-2] > 70 and rsi.iloc[-1] < rsi.iloc[-2] and  # RSI 高檔轉弱
        tmo.iloc[-2] > 0 and tmo.iloc[-1] < tmo.iloc[-2] and tmo.iloc[-1] > 0 and  # TMO 死叉但仍在0上方
        obv.iloc[-1] < obv.iloc[-2] < obv.iloc[-3] and obv.iloc[-1] > 0 and  # OBV 三連跌且仍為正
        df['close'].iloc[-1] < df['close'].iloc[-2] and  # 近兩根價格轉弱
        abs(df['close'].iloc[-1] - latest_vwap) / latest_vwap < 0.02 and  # VWAP 貼近（<2%）
        df['close'].iloc[-1] < df['open'].iloc[-1]  # K棒為陰線
    ):
        if symbol in observed_candidates:
            first = observed_candidates[symbol]
            price_diff = abs(price - first["price"]) / first["price"]
            ...
            if price_diff <= 0.02 and time_diff <= 30:
                ...
                signal_note = (
                    f"🐻 **潛伏空頭（正式建倉）** 🐻 {symbol}\n"
                    f"📉 價格：${price:.2f}｜K棒：陰線｜技術轉弱\n"
                    f"📊 RSI：{rsi.iloc[-1]:.1f} ⬇️｜TMO：{tmo.iloc[-1]:.2f} ⬇️（尚未跌破0）｜OBV：三連跌\n"
                    f"📏 VWAP 偏離：{abs(df['close'].iloc[-1] - latest_vwap) / latest_vwap:.2%}（貼近主力成本）\n"
                    f"💰 建倉金額：${capital_used:,.0f}｜張數：{shares} 股｜剩餘資金：${remaining_capital:,.0f}\n"
                    f"📌 技術面轉弱，主力可能出貨，空單進場成立\n"
                    f"🕒 時間：{now.strftime('%Y-%m-%d %H:%M:%S')}"
                )

                # ✅ 安全檢查
                if not is_safe_entry(rsi.iloc[-1], price, latest_vwap, direction="short", symbol=symbol):
                    return
...
def fetch_stock_data(symbol):
    try:
        ...
        if now_est < market_open:
            print(f"[SKIP] {symbol} 尚未開盤（{now_est.strftime('%H:%M:%S')}），跳過")
            return None
        if now_est > market_close:
            print(f"[SKIP] {symbol} 已收盤（{now_est.strftime('%H:%M:%S')}），跳過")
            return None

        # ✅ 正常盤中，往前抓 2 小時
        end_time = now_est - timedelta(minutes=5)  # 保險延遲 5 分鐘，避免最新K線還未生成
        start_time = end_time - timedelta(minutes=15)

        from_ts = int(start_time.timestamp())
        to_ts = int(end_time.timestamp())

        print(f"[DEBUG] {symbol} 從：{from_ts} 到：{to_ts}")
        ...
        for attempt in range(3):
            try:
                ...
                print(f"[RETRY-{attempt+1}] {symbol} API 錯誤，1 秒後重試：{api_error}")
                time.sleep(1)
        else:
            print(f"[ERROR] {symbol} API 連續失敗三次，跳過")
            return None
        ...
        bars = getattr(aggs, "results", [])
        if not bars:
            print(f"[WARNING] {symbol} 無有效 bars 資料，可能無成交或非主板股票")
            return None
        ...
        if not df.empty:
            ...
            print(f"[WARNING] {symbol} bars 解析後為空")
            return None
...
def analyze_stock_data(symbol, bars, tick_value, trin_value):
    try:
        ...
        if (
            ...
            shares = int(capital_used / latest_price)
            remaining_capital = capital_left - capital_used
            ...
            capital_left -= capital_used
            print(f"[建倉紀錄] {symbol} 空頭建倉於 {latest_price:.2f}｜投入資金 ${capital_used:.2f}")
    except Exception as e:
