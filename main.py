...
def calculate_exit_metrics(entry_price, exit_price, shares, entry_time, exit_time, direction="多"):
    ...
    try:
        ...
        # ✅ 根據方向計算損益與報酬率
        pnl = (exit_total - entry_total) if direction == "多" else (entry_total - exit_total)
           ...
           # === 6. 抓技術指標資料
            ...
            if signal_type2 in ["BUY", "SELL"]:
                ...
                if can_enter_new_position(symbol, capital_required):
                    ...
                    positions[symbol] = {
                        "entry_price": latest_price,        # ✅ 用 latest_price
                        "direction": direction,
...
def check_exit_and_notify(symbol, latest_price):
    ...
    pnl = (latest_price - entry_price) * exit_qty if direction == "多" else (entry_price - latest_price) * exit_qty
    profit_dollar = round(pnl, 2)
    return_pct = round(return_rate * 100, 2)
    ...
    print(f"[EXIT] {symbol} 出場 {direction}單，數量={exit_qty}，價格={latest_price:.2f}，報酬率={return_pct:.2f}%")
    ...
    content = (
        ...
        f"📊 報酬率：{return_pct:.2f}% ｜ 獲利金額：${profit_dollar:.2f}\n"
        f"🔄 原因：{reason}\n"
        f"🕒 時間：{time_str}"
    )
    requests.post(WEBHOOK_URL, json={"content": content})
...
def send_entry_push(symbol, direction, strategy_name, strategy_emoji, confidence_score, latest_price, capital_used, capital_left, shares, rsi, ema5, ema20, zscore, vwap, bb_upper, bb_lower, volume_ratio, obv_change_text, trend_note, emoji_trend, trend_score, rrov_score, mean_score):
    # ✅ 技術指標整合推播內容
    tech_info = (
        f"[技術傾向] {emoji_trend} 技術{trend_note}（僅供參考）|{symbol} ➜\n"
        f"RSI={safe_round(rsi)}｜EMA5={safe_round(ema5)}｜EMA20={safe_round(ema20)}｜"
        f"Z-score={safe_round(zscore)}｜VWAP={safe_round(vwap)}\n"
    ...
    message = (
        ...
        f"[調試] {symbol} ➜ 策略名稱：{strategy_name}|emoji：{strategy_emoji}\n"
        f"[資金確認] 已扣資金：${capital_used:,.2f}，剩餘資金：${capital_left:,.2f}\n"
