...
def write_trade_to_sheet(strategy_type, symbol, direction, entry_price, shares,
                         ...
                         return_rate="", holding_minutes="", profit_loss_amount=""):
    ...
    payload = {
        ...
        "zscore": round(zscore, 2) if zscore != "" else "",
        "roc": round(roc, 2) if roc != "" else "",
        "obv": int(obv) if obv != "" else "",
        "vwap": round(vwap, 2) if vwap != "" else "",
        "confidence_score": round(confidence_score, 2) if confidence_score != "" else "",
        "signal_note": signal_note,
        "datetime": now,
        "date": date_today,
        # ✅ 出場時附加欄位
        "return_rate": round(return_rate * 100, 2) if isinstance(return_rate, (int, float)) else return_rate,
        "holding_minutes": holding_minutes,
        "pnl": round(profit_loss_amount, 2) if isinstance(profit_loss_amount, (int, float)) else profit_loss_amount
    }
...
try:
    with open("positions.json", "r") as f:
        ...
        capital_left = data.get("capital_left", TOTAL_CAPITAL)
    print(f"[載入成功] 歷史持倉 {len(positions)} 檔｜剩餘資金：${capital_left:,.2f}")
except FileNotFoundError:
    positions = {}
    capital_left = TOTAL_CAPITAL
    print("[初始化] 無持倉檔案，啟動為空倉狀態")
...
def scan_market(symbol_list):
    ...
    if capital_left < MIN_REQUIRED_CAPITAL:
        print(f"[資金耗盡] 剩餘資金 ${capital_left:.2f} 已低於 ${MIN_REQUIRED_CAPITAL}，暫停掃描，等待出場...")
        return
    ...
    for symbol in symbol_list:
        try:
            ...
            # === 2. 技術指標
            df = calculate_indicators(df)   # 用 df 接住
            indicators = df
            latest_price = df['close'].iloc[-1]
        ...
        if (rsi > 60 or roc > 0.5 or ema5 > ema20 or obv_diff > 0):
            ...
            if signal_type1 is None and signal_note1 and "未進場" in signal_note1:
                ...
                content = (
                    ...
                    f"📊 布林乖離={bb_dev:.2f}%｜EMA差值={ema_diff:.2f}"
                )
...
def enter_position(symbol, price, direction, signal_note, rsi=None, zscore=None, strategy_name="未標記策略",
    ...
    print("📌 測試建倉成功進入 enter_position() 函數！")
    ...
    try:
        ...
        print(f"[儲存成功] 已寫入 positions.json")
    except Exception as e:
        print(f"[錯誤] 寫入持倉檔案失敗：{e}")
...
def check_exit_and_notify(symbol, latest_price):
    ...
    profit_dollar = abs(exit_qty * (latest_price - entry_price)) if direction == "多" else abs(exit_qty * (entry_price - latest_price))
     # ✅ 計算持倉時間
    hold_time = datetime.now() - datetime.fromisoformat(pos["entry_time"]) if isinstance(pos["entry_time"], str) else datetime.now() - pos["entry_time"]
    # ✅ 回收資金
    ...
    write_trade_to_sheet(
        ...
        shares=exit_qty,
        invested_capital=capital_used,
        rsi=0, zscore=0, roc=0, obv=0, vwap=0,  # 出場時不記錄技術指標
        confidence_score=0,
        signal_note=reason,
        sheet_webhook_url="https://script.google.com/macros/s/AKfycbw1XkrMXXS0dPKu1Elok9LUJIgYMkpBh4NtbfIVYIyK0b_TiycsxF7TJoWNql0b-wAj/exec",
        return_rate=return_rate,
        holding_minutes=int(hold_time.total_seconds() // 60),
        profit_loss_amount=profit_dollar
    )
    ...
    try:
        ...
        print(f"[儲存成功] 出場後已更新 positions.json")
    except Exception as e:
        print(f"[錯誤] 出場儲存失敗：{e}")
...
def main_loop():
    ...
    # ✅ 加入測試模式判斷
    if not TEST_MODE:
        if now_est.hour < 9 or (now_est.hour == 9 and now_est.minute < 30) or now_est.hour >= 16:
            print("[INFO] 非美股盤中時間，跳過掃描")
            return
...
if __name__ == "__main__":
    if TEST_MODE:
        ...
        for symbol, pos in positions.items():
            print(f"- {symbol}｜方向：{pos['direction']}｜股數：{pos['shares']}｜均價：${pos['entry_price']:.2f}｜投入：${pos['capital_used']:.2f}")
        print(f"💰 剩餘資金：${capital_left:,.2f}")
