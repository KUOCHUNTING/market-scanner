...
def write_entry_to_sheet(symbol, price, direction, shares, capital, strategy, confidence):
    try:
        ...
        creds = get_credentials_from_base64("GOOGLE_SERVICE_ACCOUNT_BASE64")
        client = gspread.authorize(creds)
...
def get_fundamentals(symbol, polygon_api_key):
    try:
...
def filter_fundamentals(symbol, fundamentals):
    ...
    if mode == "主流股":
        if market_cap < 300_000_000:
            return False, f"❌ 市值過小（主流股模式）"
        if avg_volume < 5_000_000:
            return False, f"❌ 流動性不足（< 500 萬股）"
        if is_otc or price < 1:
            return False, f"⚠️ OTC 或低價股（< $1）"

    # === 小型股條件 ===
    elif mode == "小型股":
        if market_cap < 100_000_000:
            return False, f"❌ 市值過低（< 1 億）"
        if avg_volume < 100_000:
            return False, f"❌ 太冷門（< 10 萬股）"
        if price < 0.5:
            return False, f"⚠️ 超低價股（< $0.5）"

    # === 通用風險過濾 ===
    if is_delisted:
        return False, f"❌ 已下市或停牌"
    if is_recent_earning:
        return False, f"⚠️ 財報發布期波動大"

    return True, f"✅ 通過（{mode}篩選）"
# === 🛡️ 出場風控參數（含三段鎖利）===
...
def scan_market(symbol_list):
    ...
    for symbol in symbol_list:
        try:
            ...
            # === 1. 抓資料
            df_raw = fetch_stock_data(symbol, POLYGON_API_KEY)
            ...
            if 'close' not in df_raw.columns:
                print(f"[錯誤] {symbol} ➜ DataFrame 缺少 close 欄位，實際欄位：{df_raw.columns.tolist()}")
                print(df_raw.head(3))
                continue

            if len(df_raw) < 60:
                print(f"[跳過] {symbol} ➜ 資料不足（少於 60 根）")
                continue

            # === 1.5 抓基本面資料
            polygon_api_key = os.getenv("POLYGON_API_KEY")
            if not polygon_api_key:
                raise ValueError("❌ 缺少 Polygon API 金鑰，請設定 POLYGON_API_KEY")

            fundamentals = get_fundamentals(symbol, polygon_api_key)
            ...
            if not passed:
                ...
                print(f"[基本面通過] {symbol} ➜ {reason}")

            # === 2. 技術指標分析
            indicators = calculate_indicators(df_raw)
