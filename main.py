...
def get_tick_series(minutes=30):
    try:
        ...
        from_str = start_time.isoformat().replace("：", ":")
        to_str = end_time.isoformat().replace("：", ":")

        print(f"[DEBUG] TICK from: {from_str} to: {to_str}")
...
def fetch_stock_data(symbol):
    try:
        ...
        if end_time < market_open or start_time > market_close:
            print(f"[SKIP] {symbol} 不在交易時段（{now_est.strftime('%H:%M:%S')}），跳過")
            return None

        client = RESTClient(api_key=POLYGON_API_KEY)

        # ✅ 使用 strftime 強制格式化為半形冒號格式（保險）
        from_str = start_time.strftime("%Y-%m-%dT%H:%M:%S%z")
        to_str = end_time.strftime("%Y-%m-%dT%H:%M:%S%z")

        print(f"[DEBUG] {symbol} from: {from_str} to: {to_str}")
        ...
        try:
            ...
            print(f"[ERROR] {symbol} 抓取失敗：{api_error}")
            return None
