...
# ✅ Step 5: 拿 emoji 版策略名稱
strategy_display = get_strategy_display(strategy_name)

# ✅ Step 5.1: 依策略選擇對應的條件字典
if strategy_name == "RROV":
    selected_conditions = rrov_conditions
elif strategy_name == "均值回歸":
    selected_conditions = mean_revert_conditions
elif strategy_name == "順勢策略":
    selected_conditions = trend_follow_conditions
else:
    selected_conditions = {}

# ✅ Step 5.2: 組合條件命中描述（推播用）
matched_conditions = [k for k, v in selected_conditions.items() if v]
signal_note = "｜".join(matched_conditions) if matched_conditions else "策略命中｜無條件"
...
if __name__ == "__main__":
    symbol_list = load_stock_list()
    import random
    random.shuffle(symbol_list)  # ✅ 每次啟動前打亂順序
    print(f"[啟動] 股票清單載入完畢，共 {len(symbol_list)} 檔")
