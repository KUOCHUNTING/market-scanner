...
def apply_indicators(df):
    ...
    # ✅ 新增這行：過濾資料不足的情況
    if len(df) < 15:
        print(f"[略過] 資料太少（僅 {len(df)} 筆），跳過技術指標計算")
        return None
...
def get_tick_data_from_polygon(api_key, window=20):
    ...
    try:
        ...
        if now < market_open or now > market_close:
            print(f"[SKIP] 現在是 {now.strftime('%H:%M')} ⛔ 不在盤中，TICK 不更新")
            return None

        # ✅ 抓最近 window 根 1 分鐘 K 線
...
def classify_signal(df, symbol):
    ...
    if (
        ...
        obv.iloc[-1] < 0 and abs(obv.iloc[-1]) < abs(obv.max()) * 0.06 and
        vwap.iloc[-1] > vwap.iloc[-2] and
        price < vwap.iloc[-1] and abs(price - vwap.iloc[-1]) / vwap.iloc[-1] < 0.06 and
        roc.iloc[-1] > roc.iloc[-2] and roc.iloc[-1] < 0
    ):
        message = (
    ...
    if (
        rsi.iloc[-1] < rsi.iloc[-2] and
        obv.iloc[-1] < obv.iloc[-2] and obv.iloc[-1] > 0 and obv.iloc[-1] < obv.max() * 0.06 and
        vwap.iloc[-1] < vwap.iloc[-2] and
        price > vwap.iloc[-1] and abs(price - vwap.iloc[-1]) / vwap.iloc[-1] < 0.06 and
        roc.iloc[-1] < roc.iloc[-2] and roc.iloc[-1] > 0
    ):
...
def main():
    ...
    for symbol in stock_list:
        ...
        # ✅ 新增這段，保護 check_tick_resonance() 回傳 None 時不會報錯
        tick_view = None
        try:
            tick_result = check_tick_resonance()
            if tick_result is not None:
                tick_view = tick_result
            else:
                print(f"[TICK] ❌ 無法取得共振資料")
        except Exception as e:
            print(f"[TICK 判斷錯誤] {e}")
