...
# ✅ 自動轉換策略名稱為 emoji 版
strategy_display = strategy_label_map.get(normalize_strategy_name(strategy_name), "📌 未知策略")
# === 決定策略名稱（這是你根據條件選出來的）
strategy_name = "均值回歸"  # or RROV / 順勢策略

# ✅ 正確放這裡！策略名稱存在後再做 emoji 對照
strategy_display = strategy_label_map.get(normalize_strategy_name(strategy_name), "📌 未知策略")
...
record_entry_position(
    ...
    direction=direction,
    shares=quantity,
    strategy_name=strategy_name,  # ✅ 保留原始策略名稱記錄
    confidence_score=confidence_score,
    capital_used=capital_used
)
...
write_entry_to_sheet(
    ...
    strategy=strategy_name,  # ✅ Google Sheets 保留原始名稱
    confidence=confidence_score,
    capital_left=capital_left
)
...
def get_strategy_display(name):
    ...
    return emoji_map.get(normalized, "📌 未知策略")
