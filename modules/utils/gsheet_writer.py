# modules/utils/gsheet_writer.py
...
# ✅ Google Sheets 分頁連線工具
def get_sheet(tab_name: str):
    """
    根據分頁名稱取得 worksheet 物件
    """
    try:
        sheet = connect_with_base64_key(GSHEET_URL, GCP_KEY_BASE64)
        worksheet = sheet.worksheet(tab_name)
        print(f"[DEBUG] ✅ 已連線分頁 ➜ {tab_name}")
        return worksheet
    except Exception as e:
        print(f"[❌ 錯誤] 無法連線分頁 {tab_name}：{e}")
        return None
...
def write_entry_to_sheet(entry: dict, sheet, shares: int = 0):
    ...
    try:
        ...
        print(f"[✅] 成功寫入建倉資料 ➜ {entry.get('symbol', '')}")
    except Exception as e:
        print(f"[❌] 寫入建倉資料失敗 ➜ {e}")
...
def write_exit_to_sheet(exit_info: dict, sheet):
    ...
    try:
        sheet.append_row(row, value_input_option="USER_ENTERED")
        print(f"[✅] 成功寫入出場資料 ➜ {exit_info.get('symbol', '')}")
    except Exception as e:
        print(f"[❌] 寫入出場資料失敗 ➜ {e}")
...
def write_resonance_to_sheet(resonance_data: list, sheet, timestamp=None):
    """
    將類股共振資料寫入 Google Sheets（會先清空原內容）
    """
