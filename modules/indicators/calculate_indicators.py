# modules/indicators/calculate_indicators.py
import pandas as pd
from ta.momentum import RSIIndicator, ROCIndicator
from ta.trend import EMAIndicator
from ta.volatility import BollingerBands
from ta.volume import OnBalanceVolumeIndicator
from modules.utils.validate_indicators import log_invalid_indicator
...
def calculate_indicators(df, symbol=None):
    ...
    try:
        ...
        ema_trend = [
            "上彎" if e5 > 0 and e20 > 0 else "下彎" if e5 < 0 and e20 < 0 else "糾結"
            for e5, e20 in zip(ema_5_slope, ema_20_slope)
        ]
        ...
        candle_type = (
            "十字線" if body < 0.1 * (last_high - last_low)
            else "錘頭" if last_close > last_open and lower > 2 * body
            else "流星" if last_close < last_open and upper > 2 * body
            else "陽線" if last_close > last_open
            else "陰線"
        )
