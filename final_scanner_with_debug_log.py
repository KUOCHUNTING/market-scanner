...
def get_tick_data():
    try:
        ...
        latest = df["Close"].values.ravel().iloc[-1]
        slope = df["Close"].values.ravel().diff().tail(3).mean()
        percentile = (df["Close"].values.ravel() < latest).sum() / len(df["Close"].values.ravel()) * 100
        return latest, slope, percentile
    except:
        return 0, 0, 50

def calc_indicators(df):
    df["SMA20"] = df["Close"].values.ravel().rolling(20).mean()
    df["STD"] = df["Close"].values.ravel().rolling(20).std()
    df["Upper"] = df["SMA20"] + 2 * df["STD"]
    df["Lower"] = df["SMA20"] - 2 * df["STD"]
    df["Basis"] = df["SMA20"]
    rsi = df["Close"].values.ravel().rolling(21).apply(lambda x: 100 - (100 / (1 + (x.pct_change().dropna() > 0).sum() / max((x.pct_change().dropna() < 0).sum(), 1))), raw=False)
    tmo = rsi.rolling(5).mean().rolling(3).mean()
    signal = tmo.rolling(3).mean()
    df["TMO"] = tmo
    df["TMO_signal"] = signal
    ema12 = df["Close"].values.ravel().ewm(span=12, adjust=False).mean()
    ema26 = df["Close"].values.ravel().ewm(span=26, adjust=False).mean()
    macd = ema12 - ema26
    signal_macd = macd.ewm(span=9, adjust=False).mean()
    df["MACD_line"] = macd
    df["MACD_signal"] = signal_macd
    df["MACD_hist"] = macd - signal_macd
    df["TP"] = (df["High"].values.ravel() + df["Low"].values.ravel() + df["Close"].values.ravel()) / 3
    df["Cum_TPV"] = (df["TP"] * df["Volume"].values.ravel()).cumsum()
    df["Cum_Vol"] = df["Volume"].values.ravel().cumsum()
    df["VWAP"] = df["Cum_TPV"] / df["Cum_Vol"]
    df["VolAvg"] = df["Volume"].values.ravel().rolling(16).mean()
    return df
...
def main():
    ...
    for symbol in all_symbols:
        try:
            ...
            current_price = df['Close'].values.ravel().iloc[-1]
