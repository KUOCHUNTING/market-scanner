...
while True:
   ...
   def send_discord_alert(message):
    ...
    def get_tick_data():
        try:
            df = yf.download("^TICK", interval="1m", period="30m", progress=False, prepost=True)
            if df is None or df.empty:
                return None
            return df['Close'].ravel()
        except Exception as e:
    ...
    def detect_15min_entry(symbol):
        try:
            ...
            if df is None or df.empty or len(df) < 10:
                print(f"⚠️ 跳過股票 {symbol}：資料為空或不足")
                return
            print(f"✅ 已完成技術指標計算，開始偵測訊號: {symbol}")
            if df is None or df.empty or len(df) < 10:
                return False
            close=df['Close'].ravel()
            volume=df['Volume'].ravel()
    ...
    def get_tick_data():
        try:
            df = yf.download("^TICK", period="1d", interval="1m", progress=False, prepost=True)
            df.dropna(inplace=True)
            latest = df["Close"].ravel().iloc[-1]
            slope = df["Close"].ravel().diff().tail(3).mean()
            percentile = (df["Close"].ravel() < latest).sum() / len(df["Close"].ravel()) * 100
            return latest, slope, percentile
        except:
            return 0, 0, 50
    
    def calc_indicators(df):
        df["SMA20"] = df["Close"].ravel().rolling(20).mean()
        df["STD"] = df["Close"].ravel().rolling(20).std()
        df["Upper"] = df["SMA20"] + 2 * df["STD"]
        df["Lower"] = df["SMA20"] - 2 * df["STD"]
        df["Basis"] = df["SMA20"]
        rsi = df["Close"].ravel().rolling(21).apply(lambda x: 100 - (100 / (1 + (x.pct_change().dropna() > 0).sum() / max((x.pct_change().dropna() < 0).sum(), 1))), raw=False)
        tmo = rsi.rolling(5).mean().rolling(3).mean()
        signal = tmo.rolling(3).mean()
        df["TMO"] = tmo
        df["TMO_signal"] = signal
        ema12 = df["Close"].ravel().ewm(span=12, adjust=False).mean()
        ema26 = df["Close"].ravel().ewm(span=26, adjust=False).mean()
        macd = ema12 - ema26
        signal_macd = macd.ewm(span=9, adjust=False).mean()
        df["MACD_line"] = macd
        df["MACD_signal"] = signal_macd
        df["MACD_hist"] = macd - signal_macd
        df["TP"] = (df["High"].ravel() + df["Low"].ravel() + df["Close"].ravel()) / 3
        df["Cum_TPV"] = (df["TP"] * df["Volume"].ravel()).cumsum()
        df["Cum_Vol"] = df["Volume"].ravel().cumsum()
        df["VWAP"] = df["Cum_TPV"] / df["Cum_Vol"]
        df["VolAvg"] = df["Volume"].ravel().rolling(16).mean()
        return df
    ...
    def main():
        ...
        for idx, symbol in enumerate(symbols):
            ...
            try:
                ...
                data["rsi"] = RSIIndicator(close=df['Close'],  window=14).rsi()
    ...
    def main():
        ...
        for symbol in all_symbols:
            try:
                ...
                current_price = df['Close'].ravel().iloc[-1]
    ...
    def scan_symbol(symbol):
        ...
        try:
            latest_price = yf.Ticker(symbol).history(period="1d", prepost=True)["Close"].iloc[-1]
