...
def load_symbols():
    try:
        print(">>> 嘗試載入股票清單 CSV...", flush=True)
        df = pd.read_csv("filtered_us_stocks_with_name.csv")  # 確保檔名一致
        if df is None or df.empty:
            print("⚠️ 股票清單為空，請確認 CSV 內容", flush=True)
            return []
        symbols = df['symbol'].dropna().unique().tolist()
        print(f">>> 成功讀取股票清單，共 {len(symbols)} 檔", flush=True)
        return symbols
    except FileNotFoundError:
        print("❌ 找不到檔案 filtered_us_stocks_with_name.csv，請確認檔案是否存在於目錄中", flush=True)
        return []
    except Exception as e:
        print(f"❌ 載入股票清單失敗：{e}", flush=True)
        return []
...
def detect_15min_entry(symbol):
    try:
        ...
        close = df['Close']
        volume = df['Volume']
        rsi = RSIIndicator(close=close, length=14, window=14).rsi()
...
def main():
    ...
    for idx, symbol in enumerate(symbols):
        ...
        try:
            ...
            data["rsi"] = RSIIndicator(close=data["Close"], length=14, window=14).rsi()
...
def main():
    print(">>> 掃描器啟動中...", flush=True)
    try:
        print(">>> 正在執行主程式流程...", flush=True)
        run_scanner()  # 如果你主程式不是 run_scanner() 請修改此行
        print(">>> 掃描結束。", flush=True)
    except Exception as e:
        print(">>> 發生錯誤：", str(e), flush=True)
        traceback.print_exc()
