...
def adjust_position_size():
    try:
        ...
        tab = sheet.worksheet("正式出場")
        rows = tab.get_all_values()[1:]
        returns = [float(r[4]) for r in rows[-20:] if r[4].replace('.', '', 1).replace('-', '', 1).isdigit()]
        win_rate = len([r for r in returns if r > 0]) / len(returns) if returns else 0.5
        global position_size_pct
        if win_rate > 0.6:
            position_size_pct = min(0.1, position_size_pct + 0.01)
        elif win_rate < 0.4:
            position_size_pct = max(0.01, position_size_pct - 0.01)
    except Exception as e:
        print(f"⚠️ 自動倉位調整錯誤：{e}")
...
def report_status():
    try:
        ...
        symbols = load_symbols()
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        msg = f"🟢 執行中：{now}｜盤別：{session}｜掃描數量：{len(symbols)}"
        print(msg)
...
def send_preopen_summary():
    ...
    if now_et.hour == 9 and now_et.minute == 29 and now_et.second < 10:
        try:
            ...
            client = gspread.authorize(creds)
            sheet = client.open_by_url("https://docs.google.com/spreadsheets/d/14SSmjk2Ae3rqx0VyiVoVWBXpq0NVNvsLs1RWckuX4Ko/")
            def count_rows(tab): return len(sheet.worksheet(tab).get_all_values()) - 1
            summary = "\n".join([
                f"開盤前總結：",
                f"觀察訊號：多頭 {count_rows('觀察多頭')}｜空頭 {count_rows('觀察空頭')}",
                f"預警訊號：多頭 {count_rows('預警試單')}｜空頭 {count_rows('空頭預警')}",
                f"共振訊號：{count_rows('共振進場') + count_rows('共振空單')}"
            ])
