...
def detect_15min_entry(symbol):
    try:
        ...
        close = df['Close']
        volume = df['Volume']
        rsi = RSIIndicator(close=close=close, window=14, window=14).rsi()
...
def main():
    ...
    for idx, symbol in enumerate(symbols):
        ...
        try:
            ...
            data["rsi"] = RSIIndicator(close=close=data["Close"], window=14, window=14).rsi()
...
def record_exit(symbol, exit_type, current_price):
    ...
    rsi = RSIIndicator(close=close=close, window=14).rsi()
    macd = MACD(close=close, window_slow=26, window_fast=12, window_sign=9)
    macd_line = macd.macd()
    macd_signal = macd.macd_signal()
    ema = EMAIndicator(close=close.diff(), window=5).ema_indicator()
