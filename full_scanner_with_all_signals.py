...
def check_tick_triple_confluence(tick_series):
    try:
        ...
        is_extreme = perc_rank >= 0.95 or perc_rank <= 0.05
        slope = tick_series.diff().rolling(5).mean().iloc[-1]
        is_slope_strong = abs(slope) > 50
        avg_bias = tick_series.tail(10).mean()
        is_bullish = avg_bias > 600
        is_bearish = avg_bias < -600

        return is_extreme and is_slope_strong and (is_bullish or is_bearish)
    except Exception as e:
