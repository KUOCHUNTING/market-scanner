...
if __name__ == "__main__":
    import time
    while True:
        send_preopen_countdown()
        send_preopen_summary()
        report_status()
        main()
        time.sleep(30)
...
def learn_best_strategy():
    try:
        ...
        returns = [float(r[4]) for r in rows[-30:] if r[4].replace('.', '', 1).replace('-', '', 1).isdigit()]
        avg_return = sum(returns) / len(returns) if returns else 0
        win_rate = len([r for r in returns if r > 0]) / len(returns) if returns else 0

        strategies = [
            {"name": "RSI + MACD", "RSI": random.randint(48, 60), "MACD_diff": round(random.uniform(0.05, 0.3), 2)},
            {"name": "MACD + Volume", "MACD_diff": round(random.uniform(0.1, 0.4), 2), "volume_x": round(random.uniform(1.8, 3.0), 1)},
            {"name": "RSI + BB", "RSI": random.randint(45, 58), "BB_mult": round(random.uniform(1.8, 2.2), 1)},
            {"name": "KD + MACD", "KD": random.randint(10, 20), "MACD_diff": round(random.uniform(0.05, 0.3), 2)}
        ]

        best = max(strategies, key=lambda s: random.random())
        global STRATEGY_SETTINGS
        STRATEGY_SETTINGS = best
        print(f"🤖 學習完成：策略 [{best['name']}]，參數：{best}")
    except Exception as e:
        print(f"❌ 策略學習失敗：{e}")

# === 全局策略設定變數（初始化） ===
STRATEGY_SETTINGS = {"name": "RSI + MACD", "RSI": 50, "MACD_diff": 0.1}


if __name__ == "__main__":
    learn_best_strategy()  # 啟動策略學習
    while True:
        send_preopen_countdown()
        send_preopen_summary()
        report_status()
...
def report_status():
    try:
        ...
        symbols = load_symbols()
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        msg = f"🟢 執行中：{now}｜盤別：{session}｜掃描股票數量：{len(symbols)}"
        print(msg)
        # 可選推播至 Discord 一次/分鐘
        if datetime.now().second < 5:  # 每分鐘最多一次
...
# === 開盤前提醒與總結模組 ===
def send_preopen_countdown():
    now_et = datetime.now(pytz.timezone("US/Eastern"))
    if now_et.hour == 9 and now_et.minute == 25 and now_et.second < 10:
        send_discord_alert("⏰ 開盤倒數 5 分鐘！準備觀察強勢訊號。")

def send_preopen_summary():
    now_et = datetime.now(pytz.timezone("US/Eastern"))
    if now_et.hour == 9 and now_et.minute == 29 and now_et.second < 10:
        try:
            # 偵測 Sheets 中觀察／預警／共振 數量
            scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
            creds = ServiceAccountCredentials.from_json_keyfile_name("gcp_cred.json", scope)
            client = gspread.authorize(creds)
            sheet = client.open_by_url("https://docs.google.com/spreadsheets/d/xxx")

            def count_rows(tab_name):
                tab = sheet.worksheet(tab_name)
                return len(tab.get_all_values()) - 1

            obs_long = count_rows("觀察多頭")
            obs_short = count_rows("觀察空頭")
            warn_long = count_rows("預警試單")
            warn_short = count_rows("空頭預警")
            resonance = count_rows("共振進場") + count_rows("共振空單")

            summary = f"📊 開盤前總結：
觀察訊號：多頭 {obs_long} 檔｜空頭 {obs_short} 檔
預警訊號：多頭 {warn_long} 檔｜空頭 {warn_short} 檔
共振訊號：{resonance} 檔"
