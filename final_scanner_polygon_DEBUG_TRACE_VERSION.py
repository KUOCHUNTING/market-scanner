...
def scan_all_symbols(sorted_symbols):
    # 已上移至 main()
            
    symbol_volumes = []
    
    for idx, symbol in enumerate(symbols):
        print(f"🔄 嘗試抓取成交量第 {idx+1} 檔：{symbol}")
        try:
            df_vol = fetch_5min_bars(symbol)
            if df_vol is not None and len(df_vol) > 0:
                vol = df_vol['v'].iloc[-1]
                print(f"   ✔ {symbol} 成交量 = {vol}")
...
def main():
    ...
    sorted_symbols = [s for s, _ in sorted(symbol_volumes, key=lambda x: x[1], reverse=True)]
